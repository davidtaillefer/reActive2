...
from .utils import get_timezone, to_local_iso, to_utc_iso 
from dateutil import parser

hrm_bpbak = Blueprint("hrmbak", __name__)

# GET the contents of an HRM file

@hrm_bpbak.route("/hrmbak/<hrmfile>")
def readhrm(hrmfile):
    ...
    if file_extension == ".tcx":
            ...
            for field_name in record:
                ...
                if field_value != None:
                    if isinstance(field_value, datetime):
                        field_value = to_utc_iso(field_value)
                    if "local_timestamp" in field_name:
                        field_value = to_utc_iso(datetime.fromtimestamp(field_value))
        ...
        for record in messages["session_mesgs"]:
            ...
            removed = data_dict["Activities"].pop("start_time", None)
            ...
            for record in messages["lap_mesgs"]:
                ...
                for field_name in record:
                    ...
                    if (
                        ...
                        if isinstance(field_value, datetime):
                            field_value = to_utc_iso(field_value)
                            utc_ts = to_utc_iso(field_value)
                            lap["timestamp_utc"] = utc_ts
                            starttime = field_value
                        if "lat" in field_name or "long" in field_name:
                            field_value = field_value / ((2**32) / 360)
                        lap[field_name] = field_value
                lap["end_time_utc"] = to_utc_iso(datetime.fromisoformat(
                    lap["timestamp_utc"]) + timedelta(seconds=lap["total_elapsed_time"]))
                laps[li] = lap
        ...
        if has_laps: # Add blank length dict to each lap to be populated later, and create a parallel dict structure to hold lengths while processing
        ...
        flat_tracks = []
        laps = data_dict['Activities']['Lap']

        # ✅ Preprocess laps (convert timestamps ONCE)
        lap_ranges = []
        for lap_id, lap_data in laps.items():
            lap_ranges.append((
                lap_id,
                parser.isoparse(lap_data['timestamp_utc']),
                parser.isoparse(lap_data['end_time_utc'])
            ))

        for lap_id in data_dict['Activities']['Lap']:
            tracks[lap_id] = []
            
        lap_idx = 0
        num_laps = len(lap_ranges)
        ...
        for trackpoint in messages["record_mesgs"]:
            ...
            # ✅ Faster field loop
            for field_name, field_value in trackpoint.items():

                if not isinstance(field_name, str) or "unknown" in field_name:
                    continue

                if field_value is None:
                    continue

                if not isinstance(field_value, (int, float, str, datetime)):
                    continue
                ...
                elif "lat" in field_name or "long" in field_name:
                    field_value *= (360 / (2**32))  # faster than division

                track[field_name] = field_value

            # ✅ Grade calculation (optimized)
            if prev_point:
                ...
                if None not in (lat1, lon1, lat2, lon2, ele1, ele2):
                    ...
                    if dist > 0:
                        ...
                        # ✅ O(1) rolling average
                        if len(grade_window) == grade_window.maxlen:
                            grade_sum -= grade_window[0]

                        grade_window.append(grade)
                        grade_sum += grade

                        track["grade_smooth"] = round(grade_sum / len(grade_window), 2)

            else:
                first_point = {
                    "position_lat": track.get("position_lat"),
                    "position_long": track.get("position_long")
                }

            prev_point = track

            # ✅ FAST lap matching (no nested loop)
            
            if "timestamp" not in track:
                continue
            
            track_time = parser.isoparse(track["timestamp"])
            
            
            # Move forward ONLY when safe
            while lap_idx < num_laps and track_time >= lap_ranges[lap_idx][2]:
                lap_idx += 1

            # Check current lap
            if lap_idx < num_laps:
                lap_id, start, end = lap_ranges[lap_idx]

                if start <= track_time < end:
                    tracks[lap_id].append(track)

            flat_tracks.append(track)

        # ✅ Post-processing
        for track in flat_tracks:
            g = track.get("grade_smooth")
            track["grade_bucket"] = grade_bucket(g)

        min_g, max_g = grade_stats(flat_tracks)

        for lap_id in data_dict['Activities']['Lap']:
            data_dict['Activities']['Lap'][lap_id]['Track'] = tracks.get(lap_id, [])
